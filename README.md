...
1. 随机X/Y轴位置（X/Y最小值、X/Y最大值）——弹幕出现的X/Y轴位置将会在（最小值，最大值）间浮动
	- 随机X轴时沿Y轴竖直移动
	- 随机Y轴时沿X轴水平移动
	- 同时随机X、Y轴将会启用浮动淡入的方式（不水平、竖直移动）
	- 最小值最大值可为正/负数（控制方向）

![随机弹幕位置](./screenshots/randomDanmakuPositionFigure.png) 

2. 移动时长——弹幕移动耗时（1000px耗时10秒——每秒100px）
3. 初始X/Y偏移——弹幕出现时的基础偏移位置（默认在左下角（左下角为(0,0)）
4. 最小/大移动距离——弹幕移动的距离将在（最小，最大）间浮动
5. 移动距离阈值——当最小最大移动距离横跨正负值的时候，为避免random随机到的距离过小，设置移动距离阈值为200，则最小移动距离为±200，移动距离不落在(-200,200)这一区间
